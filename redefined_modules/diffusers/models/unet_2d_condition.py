...
class UNet2DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        transformer_layers_per_block: Union[int, List[int], Tuple[int]] = 1,
        ...
        **kwargs,
        ...
        # SDXL-style additional embedding (text_time): pooled text embeds + time ids
        if addition_embed_type == "text_time":
            ...
            raise ValueError(
                f"addition_embed_type {addition_embed_type} is not supported in redefined UNet"
            )
        ...
        self.text_embedding_quantizer = DummyOp()
    ...
    def forward(
        ...
        encoder_hidden_states = self.text_embedding_quantizer(encoder_hidden_states)
        ...
        emb = self.get_time_embedding(timestep, 1)

        # 1b. SDXL additional embedding: text_embeds + time_ids -> add_embedding
        if (
            getattr(self.config, "addition_embed_type", None) == "text_time"
...
class DummyOp(torch.nn.Module):
    """Dummy module for encoding an input"""
